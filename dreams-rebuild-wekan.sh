...
options=("Install Wekan dependencies" "Build Wekan" "Run Meteor for dev on http://localhost:2222" "Run Meteor for dev on http://CURRENT-IP-ADDRESS:2222" "Run Meteor for dev on http://CUSTOM-IP-ADDRESS:PORT" "Quit")
...
do
    ...
    "Run Meteor for dev on http://localhost:2222")
		WITH_API=true RICHER_CARD_COMMENT_EDITOR=false ROOT_URL=http://localhost:2222 meteor run --exclude-archs web.browser.legacy,web.cordova --port 2222
		break
		;;

    "Run Meteor for dev on http://CURRENT-IP-ADDRESS:2222")
		IPADDRESS=$(ip a | grep 'noprefixroute' | grep 'inet ' | cut -d: -f2 | awk '{ print $2}' | cut -d '/' -f 1)
		echo "Your IP address is $IPADDRESS"
		WITH_API=true RICHER_CARD_COMMENT_EDITOR=false ROOT_URL=http://$IPADDRESS:2222 meteor run --exclude-archs web.browser.legacy,web.cordova --port 2222
		break
		;;
